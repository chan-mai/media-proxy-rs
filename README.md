...
## 実行(Docker)
```
docker run -itd -p 12766:12766 ghcr.io/chan-mai/media-proxy-rs:main
```

## 実行(Linux)
例(x86_64/amd64)
```
curl -L https://github.com/chan-mai/media-proxy-rs/releases/download/nightly/media-proxy-rs_linux-amd64.gz | gzip -d > ./media-proxy-rs
...
1. `git clone https://github.com/chan-mai/media-proxy-rs && cd media-proxy-rs`
...
2. `git clone https://github.com/chan-mai/media-proxy-rs && cd media-proxy-rs`
